...
public class JasperServer { 
    ...
    public int privateExecute( Hashtable connectionParameters, String jrxmlPath, String outputPath, Hashtable parameters) throws java.lang.Exception {
        ...
        //report = (JasperReport) JRLoader.loadObject( jasperPath( jrxmlPath ) );   #For upgrade OLD ONE
        report = (JasperReport) JRLoader.loadObjectFromFile( jasperPath( jrxmlPath ) );  //For upgrade
        ...
        //For Reference http://mattjiang.blogspot.com/2007/05/easy-way-to-remove-blank-page-generated.html
        //Add new Two properties remove blank page and blank last page for pdf only
        Boolean removeLastBlankPage = Boolean.valueOf(report.getProperty("net.sf.jasperreports.export.kd.pdf.remove.last.blank.page"));
        Boolean removeBlankPage = Boolean.valueOf(report.getProperty("net.sf.jasperreports.export.kd.pdf.remove.blank.page"));
    
        if (output.equalsIgnoreCase( "pdf" )){
            if (removeBlankPage || removeLastBlankPage)
                for (Iterator<JRPrintPage> i = jasperPrint.getPages().iterator(); i.hasNext();)
                {
                    JRPrintPage page = i.next();
                    if (removeBlankPage && page.getElements().size() == 0)
                        {
                            i.remove();
                            System.out.println( "Removing Blank page" );
                        }
                    else if (!i.hasNext() && removeLastBlankPage && page.getElements().size() == 0)
                        {
                            i.remove();
                            System.out.println( "Removing Last Blank Page" );
                        }
                }
        }
    ...
    public static void main (String [] args) {
        try {
            ...
            String JASPERHOST = System.getenv("JASPERHOST");
            java.net.InetAddress localhost = java.net.InetAddress.getLocalHost();
            if ( JASPERHOST.equalsIgnoreCase("localhost") || JASPERHOST==null || JASPERHOST.isEmpty() )
                localhost = java.net.Inet4Address.getByName("localhost");
